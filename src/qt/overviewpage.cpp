...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeTripperpayAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTripperpayAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeTripperpayAmount = strAnonymizeTripperpayAmount.remove(strAnonymizeTripperpayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTripperpayAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeTripperpayAmount*COIN) nMaxToAnonymize = nAnonymizeTripperpayAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeTripperpayAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeTripperpayAmount));
        strAnonymizeTripperpayAmount = strAnonymizeTripperpayAmount.remove(strAnonymizeTripperpayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTripperpayAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeTripperpayAmount == 0){
